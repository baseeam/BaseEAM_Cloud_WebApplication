            ...
            if (File.Exists(importProfilePath))
            {
                using (var stream = new FileStream(importProfilePath, FileMode.Open, FileAccess.Read, FileShare.Read))
                {
                    using (var xlPackage = new ExcelPackage(stream))
                    {
                        ...
                        var valueItems = _valueItemRepository.GetAll().ToList();
                        var assetCategories = valueItems.Where(v => v.ValueItemCategory.Name == "Asset Category").ToList();
                        ...
                        while (true)
                        {
                            ...
                            //get and validate asset category
                            var categoryNameProperty = manager.GetProperty("Category");
                            var categoryName = categoryNameProperty.StringValue;
                            if (string.IsNullOrEmpty(categoryName))
                            {
                                ++importResult.SkippedRecords;
                                importResult.AddError(row, "The 'Category' field is required. Skipping row.");
                                ++row;
                                continue;
                            }

                            var category = assetCategories.Where(v => v.Name == categoryName).FirstOrDefault();

                            if (category == null)
                            {
                                ++importResult.SkippedRecords;
                                importResult.AddError(row, "The 'Category' field is not valid. Skipping row.");
                                ++row;
                                continue;
                            }
                            ...
                            foreach (var property in manager.GetProperties)
                            {
                                switch (property.PropertyName)
                                {
                                    case "Name":
                                        ...
                                        break;
                                    case "Category":
                                        asset.AssetCategoryId = category.Id;
                                        break;
                                    case "Type":
