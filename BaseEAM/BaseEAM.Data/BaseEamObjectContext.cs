...
namespace BaseEAM.Data
{
    ...
    public class BaseEamObjectContext : DbContext, IDbContext
    {
        ...
        public override int SaveChanges()
        {
            ...
            ProcessAutomatedAction(auditEntities);
        ...
        private void ProcessAutomatedAction(List<AuditEntity> auditEntities)
        {
            foreach(AuditEntity auditEntity in auditEntities)
            {
                if(auditEntity.Action == AuditAction.Modified)
                {
                    // check if there're configured automated actions
                    var automatedActions = this.Set<AutomatedAction>()
                        .Where(a => a.EntityType == auditEntity.EntityType.Name)
                        .ToList();
                    if(automatedActions.Count > 0)
                    {
                        foreach(var ac in automatedActions)
                        {
                            switch(ac.WhenUsed)
                            {
                                case (int?)ActionWhenUsed.EntityInserted:
                                    // check the condition of auditEntity
                                    var isNewProperty = auditEntity.Properties.SingleOrDefault(p => p.Name == "IsNew");
                                    if (isNewProperty != null)
                                    {
                                        if ((bool)isNewProperty.Original == true
                                            && (bool)isNewProperty.Current == false)
                                        {
                                            ExecuteAutomatedAction(auditEntity, ac);
                                        }
                                    }
                                    break;
                                case (int?)ActionWhenUsed.EntityUpdated:
                                    ExecuteAutomatedAction(auditEntity, ac);
                                    break;
                                case (int?)ActionWhenUsed.EntityDeleted:
                                    // check the condition of auditEntity
                                    var isDeletedProperty = auditEntity.Properties.SingleOrDefault(p => p.Name == "IsDeleted");
                                    if (isDeletedProperty != null)
                                    {
                                        if ((bool)isDeletedProperty.Original == false
                                            && (bool)isDeletedProperty.Current == true)
                                        {
                                            ExecuteAutomatedAction(auditEntity, ac);
                                        }
                                    }
