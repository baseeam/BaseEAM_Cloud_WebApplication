...
namespace BaseEAM.Data
{
    public static class SqlServerTemplate
    {
        ...
        public static readonly string AssetSearch =
            "SELECT DISTINCT Asset.Id, Asset.Name, Asset.SerialNumber, AssetType.Id, AssetType.Name, AssetStatus.Id, AssetStatus.Name, AssetCategory.Id, AssetCategory.Name, " +
            "Location.Id, Location.Name, Location.HierarchyNamePath, Site.Id, Site.Name " +
            " FROM Asset" +
            " LEFT JOIN Valueitem AS AssetCategory on Asset.AssetCategoryId = AssetCategory.Id" +
            ...
            " INNER JOIN User ON SecurityGroup_User.UserId = User.Id" +
            " /**where**/" +
            ActiveCondition("Asset", "AssetType", "AssetStatus", "AssetCategory", "Location", "Site") +
            " /**orderby**/" +
            " OFFSET @skip ROWS FETCH NEXT @take ROWS ONLY";
