...
namespace BaseEAM.Web.Controllers
{
    public class CommonController : BaseController
    {
        ...
        [HttpPost]
        public JsonResult SLADefinitions(string entityType, string param)
        {
            var items = _slaDefinitionRepository.GetAll()
                .Where(s => s.EntityType == entityType && s.Description.Contains(param));
            var choices = new List<SelectListItem>();
            foreach (var item in items)
            {
                choices.Add(new SelectListItem
